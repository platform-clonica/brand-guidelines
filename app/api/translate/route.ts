...
- On a budget page, a line or bullet that reads only \`nototal\` (or \`sin total\` / \`no total\`) is a marker that hides the total row: copy it verbatim, do NOT translate it and do NOT drop it.
...
Translate everything else into ${lang}: titles (#, ##, ### — including \`## Presupuesto\` and \`### Condiciones\`), eyebrows (UPPERCASE lines such as CONTEXTO / EL RETO), paragraphs, list items, gantt phase-row labels (the word before ":" on a phase line, copying everything after the colon verbatim — it may be a single value, a range, or a comma-separated list of spans such as \`1-3, 6, 9-10.5\`), and the unit word on a gantt axis line (e.g. \`semanas:\` → its ${lang} equivalent). In a manifesto title, the \` / palabra / \` slash-emphasis stays — keep the two slashes and translate only the word between them. Keep the translation natural and professional; never add or remove content.`;
}
